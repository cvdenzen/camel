    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() throws Exception {
                from("direct:single-codec").to("pilight:tcp://localhost:{{port}}?encoder=#encoder&sync=false");

                from("pilight:tcp://localhost:{{port}}?decoder=#decoder&sync=false").to("mock:single-codec");
            }
        };
    }
}