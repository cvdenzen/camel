 ...
 * @version 
 */
public class PilightComponentWithConfigurationTest extends CamelTestSupport {

    @Test
    public void testPilightComponentWithConfiguration() throws Exception {
        PilightComponent comp = context.getComponent("mina", PilightComponent.class);

        PilightConfiguration cfg = new PilightConfiguration();
        cfg.setTextline(true);

        comp.setConfiguration(cfg);
        assertSame(cfg, comp.getConfiguration());

        PilightEndpoint e1 = (PilightEndpoint) comp.createEndpoint("mina://tcp://localhost:4455");
        PilightEndpoint e2 = (PilightEndpoint) comp.createEndpoint("mina://tcp://localhost:5566?sync=false");
