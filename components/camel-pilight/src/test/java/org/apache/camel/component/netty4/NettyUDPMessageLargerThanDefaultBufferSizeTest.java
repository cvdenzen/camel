...
public class NettyUDPMessageLargerThanDefaultBufferSizeTest extends BaseNettyTest {
    ...
    private void sendMessage(int messageSize) throws Exception {
        ...
        template.sendBody("pilight:udp://localhost:{{port}}?sync=false", message);
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                from("pilight:udp://localhost:{{port}}?option." + ChannelOption.RCVBUF_ALLOCATOR.name() + "=#" + ChannelOption.RCVBUF_ALLOCATOR.name())
                    .to("mock:result");
            }
        };
    }
