...
public class NettyProducerHangTest extends CamelTestSupport {
    ...
    public void nettyProducerHangsOnTheSecondRequestToTheSocketWhichIsClosed() throws Exception {
        ...
        String response1 = template.requestBody("pilight:tcp://localhost:" + PORT + "?textline=true&sync=true", "request1", String.class);
        log.info("Received first response <" + response1 + ">");

        try {
            // our test server will close the socket now so we should get an error
            template.requestBody("pilight:tcp://localhost:" + PORT + "?textline=true&sync=true", "request2", String.class);
        } catch (Exception e) {
            assertStringContains(e.getCause().getMessage(), "No response received from remote server");
        }
        
        String response2 = template.requestBody("pilight:tcp://localhost:" + PORT + "?textline=true&sync=true", "request3", String.class);
        log.info("Received 2nd response <" + response2 + ">");

        try {
            // our test server will close the socket now so we should get an error
            template.requestBody("pilight:tcp://localhost:" + PORT + "?textline=true&sync=true", "request4", String.class);
        } catch (Exception e) {
            assertStringContains(e.getCause().getMessage(), "No response received from remote server");
        }
    }
