 ...
 * @version 
 */
public class PilightConsumer extends DefaultConsumer {
    private static final Logger LOG = LoggerFactory.getLogger(PilightConsumer.class);
    ...
    public PilightConsumer(final PilightEndpoint endpoint, Processor processor) {
    ...
    protected void doStart() throws Exception {
        ...
        if (protocol.equals("tcp") && clientMode) {
            ...
            // pilight
            session.write(pilightIdentification);
        } else {
            // pilight: this does not happen, as pilight is tcp
    ...
    private final class ReceiveHandler extends IoHandlerAdapter {
        ...
        public void messageReceived(IoSession session, Object object) throws Exception {
            ...
            if (sync) {
                Object body;
                if (exchange.hasOut()) {
                    body = PilightPayloadHelper.getOut(getEndpoint(), exchange);
                } else {
                    body = PilightPayloadHelper.getIn(getEndpoint(), exchange);
                }
     ...
     */
    final static String pilightIdentification = "{" +
            "\"action\":\"identify\"," +
            "\"options\":{" +
            "\"core\":1," +
            "\"receiver\": 1," +
