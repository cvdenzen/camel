 ...
 * @version 
 */
public class PilightProducer extends DefaultProducer implements ServicePoolAware {
    private static final Logger LOG = LoggerFactory.getLogger(PilightProducer.class);
    ...
    public PilightProducer(PilightEndpoint endpoint) {
        super(endpoint);
    ...
    @Override
    public PilightEndpoint getEndpoint() {
        return (PilightEndpoint) super.getEndpoint();
    }
    ...
    protected void doProcess(Exchange exchange) throws Exception {
        ...
        Object body = PilightPayloadHelper.getIn(getEndpoint(), exchange);
        if (body == null) {
        ...
        // write the body
        PilightHelper.writeBody(session, body, exchange);
        ...
        if (sync) {
            ...
            if (handler.getCause() != null) {
                ...
                if (ExchangeHelper.isOutCapable(exchange)) {
                    PilightPayloadHelper.setOut(exchange, handler.getMessage());
                } else {
                    PilightPayloadHelper.setIn(exchange, handler.getMessage());
                }
            }
        }
    }
    ...
    protected void maybeDisconnectOnDone(Exchange exchange) {
        ...
        Boolean close;
        if (ExchangeHelper.isOutCapable(exchange)) {
            close = exchange.getOut().getHeader(PilightConstants.MINA_CLOSE_SESSION_WHEN_COMPLETE, Boolean.class);
        } else {
            close = exchange.getIn().getHeader(PilightConstants.MINA_CLOSE_SESSION_WHEN_COMPLETE, Boolean.class);
        }
    ...
    private void closeConnection() {
        if (connector instanceof SocketConnector) {
            ...
            // Default worker timeout is 60 sec and therefore the client using PilightProducer cannot terminate the JVM
    ...
    private final class ResponseHandler extends IoHandlerAdapter {
        private PilightEndpoint endpoint;
        private Object message;
        private Throwable cause;
        private boolean messageReceived;

        private ResponseHandler(PilightEndpoint endpoint) {
