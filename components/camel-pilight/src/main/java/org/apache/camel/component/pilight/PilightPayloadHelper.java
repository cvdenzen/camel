 ...
 * @version 
 */
public final class PilightPayloadHelper {

    private PilightPayloadHelper() {
        //Utility Class
    }

    public static Object getIn(PilightEndpoint endpoint, Exchange exchange) {
    ...
    public static Object getOut(PilightEndpoint endpoint, Exchange exchange) {
        if (endpoint.getConfiguration().isTransferExchange()) {
            // we should transfer the entire exchange over the wire (includes in/out)
            return DefaultExchangeHolder.marshal(exchange);
        } else {
