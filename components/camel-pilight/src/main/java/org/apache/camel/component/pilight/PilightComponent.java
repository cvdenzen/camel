 ...
 * @version 
 */
public class PilightComponent extends UriEndpointComponent {
    private static final Logger LOG = LoggerFactory.getLogger(PilightComponent.class);
    @Metadata(label = "advanced")
    private PilightConfiguration configuration;

    public PilightComponent() {
        super(PilightEndpoint.class);
    }

    public PilightComponent(CamelContext context) {
        super(context, PilightEndpoint.class);
    }
    ...
    protected Endpoint createEndpoint(String uri, String remaining, Map<String, Object> parameters) throws Exception {
        ...
        // we need to copy or create a new PilightConfiguration here
        PilightConfiguration config;
        if (configuration != null) {        
            config = configuration.copy();
        } else {
            config = new PilightConfiguration();
        }
    ...
    public Endpoint createEndpoint(PilightConfiguration config) throws Exception {
        return createEndpoint(config.getUriString(), config);
    }

    private Endpoint createEndpoint(String uri, PilightConfiguration config) throws Exception {
    ...
    protected PilightEndpoint createVmEndpoint(String uri, PilightConfiguration configuration) {
        boolean minaLogger = configuration.isPilightLogger();
        ...
        // connector config
        configureCodecFactory("PilightProducer", connector.getDefaultConfig(), configuration);
        if (minaLogger) {
        ...
        configureCodecFactory("PilightConsumer", acceptor.getDefaultConfig(), configuration);
        if (minaLogger) {
            acceptor.getFilterChain().addLast("logger", new LoggingFilter());
        }
        appendIoFiltersToChain(filters, acceptor.getFilterChain());

        PilightEndpoint endpoint = new PilightEndpoint(uri, this);
    ...
    protected PilightEndpoint createSocketEndpoint(String uri, PilightConfiguration configuration) {
        boolean minaLogger = configuration.isPilightLogger();
        ...
        ExecutorService acceptorPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightSocketAcceptor");
        ExecutorService connectorPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightSocketConnector");
        ExecutorService workerPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightThreadPool");
        ...
        // must use manual thread model according to Pilight documentation
        connectorConfig.setThreadModel(ThreadModel.MANUAL);
        configureCodecFactory("PilightProducer", connectorConfig, configuration);
        ...
        configureCodecFactory("PilightConsumer", acceptorConfig, configuration);
    ...
    protected void configureCodecFactory(String type, IoServiceConfig config, PilightConfiguration configuration) {
    ...
    protected void configureDefaultCodecFactory(String type, IoServiceConfig config, PilightConfiguration configuration) {
    ...
    protected PilightEndpoint createDatagramEndpoint(String uri, PilightConfiguration configuration) {
        boolean minaLogger = configuration.isPilightLogger();
        long timeout = configuration.getTimeout();
        ...
        ExecutorService acceptorPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightDatagramAcceptor");
        ExecutorService connectorPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightDatagramConnector");
        ExecutorService workerPool = getCamelContext().getExecutorServiceManager().newCachedThreadPool(this, "PilightThreadPool");
        ...
        // must use manual thread model according to Pilight documentation
        connectorConfig.setThreadModel(ThreadModel.MANUAL);
        configureDataGramCodecFactory("PilightProducer", connectorConfig, configuration);
        connectorConfig.getFilterChain().addLast("threadPool", new ExecutorFilter(workerPool));
        if (minaLogger) {
            connectorConfig.getFilterChain().addLast("logger", new LoggingFilter());
        }
        ...
        // must use manual thread model according to Pilight documentation
        acceptorConfig.setThreadModel(ThreadModel.MANUAL);
        configureDataGramCodecFactory("PilightConsumer", acceptorConfig, configuration);
        acceptorConfig.setDisconnectOnUnbind(true);
     ...
     */
    protected void configureDataGramCodecFactory(final String type, final IoServiceConfig config, final PilightConfiguration configuration) {
        ProtocolCodecFactory codecFactory = configuration.getCodec();
        if (codecFactory == null) {
            codecFactory = new PilightUdpProtocolCodecFactory(getCamelContext());
    ...
    private static Charset getEncodingParameter(String type, PilightConfiguration configuration) {
