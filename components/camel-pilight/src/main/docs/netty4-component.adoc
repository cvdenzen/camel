...
The *pilight* component in Camel is a socket communication component,
...
<dependency>
    ...
    <artifactId>camel-pilight</artifactId>
...
[source,text]
----
pilight:tcp://localhost:99999[?options]
pilight:udp://remotehost:99999/[?options]
----
...
The Pilight component supports 5 options which are listed below.
...
The Pilight endpoint is configured using URI syntax:

----
pilight:protocol:host:port
----
...
RouteBuilder builder = new RouteBuilder() {
  public void configure() {
    from("pilight:udp://localhost:5155?sync=true")
...
RouteBuilder builder = new RouteBuilder() {
  public void configure() {
       from("pilight:tcp://localhost:5150")
           .to("mock:result");
  }
};
----
...
[[Pilight-UsingtheJSSEConfigurationUtility]]
...
[[Pilight-Programmaticconfigurationofthecomponent]]
...
NettyComponent nettyComponent = getContext().getComponent("pilight", NettyComponent.class);
nettyComponent.setSslContextParameters(scp);
----

[[Pilight-SpringDSLbasedconfigurationofendpoint]]
...
...
  <to uri="pilight:tcp://localhost:5150?sync=true&ssl=true&sslContextParameters=#sslContextParameters"/>
...
----

[[Pilight-UsingBasicSSL/TLSconfigurationontheJettyComponent]]
...
context.addRoutes(new RouteBuilder() {
  public void configure() {
      String netty_ssl_endpoint =
         "pilight:tcp://localhost:5150?sync=true&ssl=true&passphrase=#password"
...
[[Pilight-GettingaccesstoSSLSessionandtheclientcertificate]]
...
<<pilight-component,Pilight>> component will store the `SSLSession` as a
...
<util:list id="decoders" list-class="java.util.LinkedList">
        <bean class="org.apache.camel.component.pilight.ChannelHandlerFactories" factory-method="newLengthFieldBasedFrameDecoder">
    ...
    <bean id="length-decoder" class="org.apache.camel.component.pilight.ChannelHandlerFactories" factory-method="newLengthFieldBasedFrameDecoder">
        <constructor-arg value="1048576"/>
        <constructor-arg value="0"/>
        <constructor-arg value="4"/>
        <constructor-arg value="0"/>
...
<camelContext id="multiple-netty-codecs-context" xmlns="http://camel.apache.org/schema/spring">
    <route>
        ...
        <to uri="pilight:tcp://localhost:5150?encoders=#encoders&amp;sync=false"/>
    </route>
    <route>
        <from uri="pilight:tcp://localhost:5150?decoders=#length-decoder,#string-decoder&amp;sync=false"/>
...
[source,java]
----
from("pilight:tcp://localhost:8080").process(new Processor() {
    public void process(Exchange exchange) throws Exception {
...
[[Pilight-Addingcustomchannelpipelinefactoriestogaincompletecontroloveracreatedpipeline]]
...
context.addRoutes(new RouteBuilder() {
  public void configure() {
      String netty_ssl_endpoint =
         "pilight:tcp://localhost:5150?serverInitializerFactory=#spf"
      String return_string =
         "When You Go Home, Tell Them Of Us And Say,"
         + "For Your Tomorrow, We Gave Our Today.";
...
`org.apache.camel.component.pilight.NettyServerBossPoolBuilder` builder
for Netty consumers, and a
`org.apache.camel.component.pilight.NettyClientBossPoolBuilder` for the
Netty producers.
...
----
<route>
  <from uri="pilight:tcp://localhost:5021?textline=true&amp;sync=true&amp;workerPool=#sharedPool&amp;usingExecutorService=false"/>
  <to uri="log:result"/>
  ...
</route>
----
...
----
<route>
  <from uri="pilight:tcp://localhost:5022?textline=true&amp;sync=true&amp;workerPool=#sharedPool&amp;usingExecutorService=false"/>
  <to uri="log:result"/>
  ...
</route>
----
...
=== A publish/subscribe client

A typical publish/subscribe client will write the subscription data to the server and will then wait for messages
to be published to it.
Up to now (Camel v 2.21.1) there is no way to do this in Camel.
A solution could be to share a channel between a producer (e.g. in a Java DSL to() method) and a consumer (e.g. in a
Java DSL from() method). The producer and the consumer will both open the same channel (and hence the same socket connection).
The producer will then send the subscription data to the server and it will quit. The channel will still be alive because
it is also used by a consumer (possibly in another route).
To make it possible to use multiple publish/subscribe servers, the sharedChannels are named.
